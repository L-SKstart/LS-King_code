...
> 🔄 同步自 `/memories/reasonix-workspace.md` — 2026-06-14 18:42
...
1. **修改文件后必须告知"已修改"，提示推送Git创建版本（不限定虚拟环境；根据当前工作环境说明下一步）**，列出文件清单
...
15. **🆕 记忆强制单一化**：工作空间仅`memory.md`为三方唯一共读记忆文件；Reasonix侧记忆已归并；**规则/记忆/规范的新增权归用户，AI不可自行创建新文件，仅可更新已有文件**


## 报错处理流程（6步）
Step1 识别关键错误行 → Step2 提取关键信息 → Step3 读取相关配置文件 → Step4 分析根因给出方案 → Step5 告知"已修改"，提示推送Git创建版本（不限定环境） → Step6 记录到错误分类文档
...
3. 一劳永逸优先
4. 步骤可复现
5. 修改必告知"已修改"并提示下一步（推送Git/重新上传等）
6. 问题必记录
