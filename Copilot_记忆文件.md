...
> 🔄 同步自 `/memories/reasonix-workspace.md` — 2026-06-14 16:18
...
**🆕 所有回复和操作内容必须严格按用户要求执行，用户未要求的内容（分析、建议、命令、解释等）不得擅自添加。**

## 7 个核心规范文件（第零条强化：每次回复前必须全读）
1. `需求要求记录.md` — 硬性约束 + 沟通约定（含第零条至高规则）
2. `工作规范与路径注册表.md` — 路径注册 + 报错处理流程 + 通用约束
3. `错误分类记录.md` — 按类型归档所有已解决报错
4. `AI协作团队身份与分工.md` — 身份区分与分工边界
...
8. **🚨 做了操作必须 @Copilot @Claude 告知所有人**：先读chat.md确认末尾→严格**追加**（不可插入/倒序/修改已有内容）→格式：`### 图标 名称：[日期 时间] 中文主题摘要`→内容→`⏱️`→`---`，**标题必须含主题摘要，不可仅写时间戳**
9. **操作记录按需生成**：仅代码/报错/服务器操作生成；纯咨询/规则调整/日常维护不生成
10. **开场问好+对时**：每条回复都必须开场问好+自报身份+对时（本机电脑时间），不可遗漏
11. **图片中转处理**：Reasonix 不能读图 → 用户发给 Copilot/Claude → 读取后写入 chat.md 并 @Reasonix
12. **文件优化防冗余**：规则以 `需求要求记录.md` 为唯一源；chat.md 超500行归档；操作记录按月合并
13. **🆕 Skill必须含中文用法指南**：概述+调用方式+示例+输入输出；同时更新 `AI_Skills使用文档.md`
14. **🆕 读到同事内容必须回复 + chat.md超1000行自动调用/chat-archive归档**
15. **🆕 记忆强制单一化**：Reasonix记忆永久冻结3文件（team-and-rules/workspace-state/credentials-all）；工作空间`memories/WORKSPACE.md`为唯一速查文件；**规则/记忆/规范的新增权归用户，AI不可自行创建新文件，仅可更新已有文件**
...
- ✅ 2026-06-14 15:00启动，16:05全部完成，四服务正常运行
...
### Skill 文件体系（11个，2026-06-14 16:53 全部已读取用法指南）

| # | Skill | 类型 | 调用方式 | 用途 |
|---|-------|------|----------|------|
| 1 | `diagnose` | 子代理 | `/diagnose <报错>` | 一键诊断部署报错（含凭据⚠️） |
| 2 | `deploy-check` | 子代理 | `/deploy-check` | 一键巡检5项服务（含凭据⚠️） |
| 3 | `chat-archive` | 子代理 | `/chat-archive` | chat.md超1000行自动归档 |
| 4 | `op-log` | 内联 | `/op-log <摘要>` | 操作记录模板生成 |
| 5 | `token-saver` | 内联 | `/token-saver lite/full/ultra` | Token精简50-70% |
| 6 | `token-budget-advisor` | 内联 | 自动触发 | 回复前估算token+选深度 |
| 7 | `git-helper` | 内联 | `/git-helper commit/pr/log` | Git操作辅助 |
| 8 | `code-review` | 内联 | `/code-review <文件>` | 四维代码审查 |
| 9 | `file-organizer` | 内联 | `/file-organizer tree/classify` | 文件整理 |
| 10 | `learn-topic` | 子代理 | `/learn-topic <主题>` | 主题研究→结构化笔记 |
| 11 | `context-optimizer` | 子代理 | 自动触发 `/context-optimizer` | 上下文体检防token膨胀 |

**Copilot调用方式：** 直接读取 `skills/reasonix/<skill名>/SKILL.md` → 「用法指南」章节
**详细指南：** `/memories/skills-usage-guide.md`（已持久化存储）

### Git 备份工作流
- 仓库：`github.com/L-SKstart/LS-King_code`，主线：`workspace`（唯一工作分支）
- **旧分支**（已归档不动）：`main`、`workspace-backup`
- **提交规则**：只推 `workspace` 分支
- 排除（`.gitignore`）：`认证信息配置.md`、含凭据的 skill 文件
- 回滚：`git clone` → 除凭据外全部恢复

### 记忆存储策略（第15条规则）
- **主副本**：`Copilot_记忆文件.md`（工作空间物理文件，Git备份）
- **缓存副本**：`/memories/reasonix-workspace.md`（Copilot快速加载）
- **同步规则**：任何记忆更新 → 同时更新两个副本
- **Reasonix侧**：记忆永久冻结3文件（team-and-rules / workspace-state / credentials-all），禁止新建
- **工作空间**：`memories/WORKSPACE.md` 为唯一速查文件（2KB，团队+规则+路径+凭据+Skill+流程）
- **chat.md归档**：超500行建议、超1000行强制归档 `chat_archive_*.md`
