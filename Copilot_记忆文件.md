...
8. **🚨 做了操作必须 @所有人 告知**：文件修改、Git推送、记忆更新、部署执行等，同步到 `chat.md` 并 @相关AI，不可悄悄执行
9. **任何新增内容必须通知 chat.md**：新增要求、记忆更新、新文件创建、规则变更、配置修改等，都必须在 `chat.md` 中 @对方 告知
10. **开场问好+对时**（本机电脑时间），不可遗漏
11. **图片中转处理**：Reasonix不能读图→发给Copilot/Claude→读取后写chat.md@Reasonix
12. **文件优化防冗余**：规则以需求要求记录.md为唯一源；chat.md超500行归档；操作记录按月合并：每条回复都必须开场问好+自报身份+对时（本机电脑时间），不可遗漏
...
### Git 备份工作流
- 仓库：`github.com/L-SKstart/LS-King_code`，主线：`main`，备份分支：`workspace-backup`
- **提交规则**：日常只推 `workspace-backup`，不直推 `main`
- 排除：`认证信息配置.md`（含凭据）
