---
description: 默认开启。回复前估算token用量，让用户选择回复深度四档，精准控制token消耗。可手动关闭。

# Token Budget Advisor — Token 预算顾问

> <font color="red">**⚡ 默认开启**</font> — 每次对话自动激活。说"关闭预算"或"/token-budget-advisor off"可关闭。

在每次回复前拦截，让用户选择回复深度后再回答，精准控制 token 消耗。

## 默认行为

- <font color="red">**每次对话开始时自动激活**</font>，无需手动调用
- 每次回复前展示深度选项（除非用户已在本会话中选择过级别）
- 用户说"关闭预算"或"关闭token控制"→ 本次会话关闭
- 用户说"开启预算"→ 重新激活
...
****调用方式：** <font color="red">**默认开启，每次对话自动激活**</font>。说"关闭预算"暂停，"开启预算"恢复。
